 ...
 * $dbuser y $dbpass. */
	...
	if (! $dbh = mysqli_connect($dbhost, $dbuser, $dbpass, $dbname)) {
		if ($DB_DEBUG) {
			...
			echo "<p><b>Error de MySQL</b>: ", mysql_error($dbh);
		} else {
 ...
 * en la base de datos */
	...
	$qid = mysqli_query($GLOBALS['lnk'],$query);

	if (! $qid && ! $silent) {
		if ($DB_DEBUG) {
			echo "<h2>No pude ejecutar la consulta</h2>";
			echo "<pre>" . htmlspecialchars($query) . "</pre>";
			echo "<p><b>Error de MySQL</b>: ", mysql_error($GLOBALS['lnk']);
		} else {
			echo "<h2>Error en la Base de Datos</h2>";
		}
 ...
 * query */
	...
	return mysqli_affected_rows($GLOBALS['lnk']);
}
 ...
 * function to give you the ID of the new autonumber value */
	...
	return mysqli_insert_id($GLOBALS['lnk']);
}
